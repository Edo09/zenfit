    ...
    <View className="flex-1 bg-gray-50">
      ...
      <View className="flex-row gap-2 px-4 pt-4 pb-2">
        <Pressable
          ...
          className={`rounded-full px-4 py-2 ${activeFilter === "all" ? "bg-green-600" : "bg-white border border-gray-200"}`}
        >
          <Text className={`text-sm font-medium ${activeFilter === "all" ? "text-white" : "text-gray-600"}`}>
            All
          </Text>
        </Pressable>
        {MEAL_TYPES.map((type) => (
          <Pressable
            ...
            className={`rounded-full px-4 py-2 ${activeFilter === type ? "bg-green-600" : "bg-white border border-gray-200"}`}
          >
            <Text className={`text-sm font-medium capitalize ${activeFilter === type ? "text-white" : "text-gray-600"}`}>
              {type}
            </Text>
          </Pressable>
        ))}
